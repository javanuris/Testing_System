...
public class MySqlTestDao extends BaseDao<Test> implements TestDao {
    ...
    private static final String FIND_TEST_BY_ANSWER = "SELECT tests.test_id , tests.name , tests.timing , tests.percentage FROM answers INNER JOIN questions ON questions.question_id  = answers.question_id INNER JOIN tests ON questions.test_id  = tests.test_id WHERE answers.answer_id = ?";
    ...
    private Test itemTest(Test test, ResultSet resultSet) throws SQLException {
        ...
        test.setPercentage(resultSet.getInt(4));
        return test;
    }
    ...
    private PreparedStatement statementTest(PreparedStatement statement, Test item) throws SQLException {
        ...
        statement.setInt(3 , item.getPercentage());
        return statement;
    }
